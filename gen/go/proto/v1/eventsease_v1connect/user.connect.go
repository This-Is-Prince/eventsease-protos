...
const (
	...
	UserServiceName = "eventsease.v1.UserService"
)
...
// period.
const (
	// UserServiceCheckUserProcedure is the fully-qualified name of the UserService's CheckUser RPC.
...
func NewUserServiceClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) UserServiceClient {
	...
	return &userServiceClient{
		checkUser: connect.NewClient[v1.CheckUserRequest, v1.CheckUserResponse](
			httpClient,
			baseURL+UserServiceCheckUserProcedure,
			connect.WithSchema(userServiceMethods.ByName("CheckUser")),
			connect.WithClientOptions(opts...),
		),
		registerFCM: connect.NewClient[v1.RegisterFCMRequest, v1.RegisterFCMResponse](
			httpClient,
			baseURL+UserServiceRegisterFCMProcedure,
...
func (c *userServiceClient) CheckUser(ctx context.Context, req *connect.Request[v1.CheckUserRequest]) (*connect.Response[v1.CheckUserResponse], error) {
	return c.checkUser.CallUnary(ctx, req)
}

// RegisterFCM calls eventsease.v1.UserService.RegisterFCM.
func (c *userServiceClient) RegisterFCM(ctx context.Context, req *connect.Request[v1.RegisterFCMRequest]) (*connect.Response[v1.RegisterFCMResponse], error) {
...
func NewUserServiceHandler(svc UserServiceHandler, opts ...connect.HandlerOption) (string, http.Handler) {
	userServiceMethods := v1.File_proto_v1_user_proto.Services().ByName("UserService").Methods()
	userServiceCheckUserHandler := connect.NewUnaryHandler(
		UserServiceCheckUserProcedure,
		svc.CheckUser,
		connect.WithSchema(userServiceMethods.ByName("CheckUser")),
		connect.WithHandlerOptions(opts...),
	)
	userServiceRegisterFCMHandler := connect.NewUnaryHandler(
		UserServiceRegisterFCMProcedure,
		svc.RegisterFCM,
		connect.WithSchema(userServiceMethods.ByName("RegisterFCM")),
		connect.WithHandlerOptions(opts...),
	)
...
func (UnimplementedUserServiceHandler) CheckUser(context.Context, *connect.Request[v1.CheckUserRequest]) (*connect.Response[v1.CheckUserResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("eventsease.v1.UserService.CheckUser is not implemented"))
}
